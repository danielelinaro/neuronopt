...
def compute_fI_curve_hall_of_fame(I, swc_file, mech_file, hof_file='hall_of_fame.pkl', \
    ...
    f = np.zeros((len(hall_of_fame),len(I)))
    no_spikes = np.zeros((len(hall_of_fame),len(I)))
    inverse_first_isi = np.zeros((len(hall_of_fame),len(I)))
    inverse_last_isi = np.zeros((len(hall_of_fame),len(I)))
    ...
    plt.legend(loc='best')
    plt.savefig('fI_curve_hall_of_fame.pdf')
    plt.show()
